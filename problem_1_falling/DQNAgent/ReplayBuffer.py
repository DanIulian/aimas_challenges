...
class ReplayBuffer(object):
    ...
    def __init__(self, size, frame_history_len):
        ...
        self.memoryName = "ReplayMemoryDump"
    ...
    def _encode_sample(self, idxes):
        ...
        done_mask = np.array(
            [0 if self.done[idx] else 1 for idx in idxes], dtype=np.uint8)
    ...
    def _encode_observation(self, idx):
        ...
        for idx in range(start_idx, end_idx - 1):
            if self.done[idx % self.size]:  #new episode
                start_idx = idx + 1
    ...
    def saveMemory(self):
        with open(self.memoryName, "wb") as f:
            param_list = [
                self.next_idx, self.num_in_buffer, self.obs, self.action,
                self.reward, self.done
            ]
            pickle.dump(param_list, f)

    def loadMemory(self):
        with open(self.memoryName, "rb") as f:
            self.next_idx, self.num_in_buffer, self.obs, self.action, self.reward, self.done = pickle.load(
                f)
